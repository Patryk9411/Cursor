#!/usr/bin/env python3
"""
Text-based Battleship (Ships) game: player vs computer.
"""
...
def place_ship(board: list[list[str]], length: int) -> None:
    """Randomly place a ship without overlapping existing ships."""
    while True:
...
def parse_move(raw: str) -> tuple[int, int] | None:
    ...
    if row_char not in ROW_LABELS or not col_part.isdigit():
        return None
...
def all_sunk(board: list[list[str]]) -> bool:
    return not any(cell == "S" for row in board for cell in row)


def computer_move(history: set[tuple[int, int]]) -> tuple[int, int]:
    while True:
...
def main() -> None:
    print("Welcome to Battleship! Sink the computer's fleet to win.\n")
    ...
    while True:
        ...
        if result == "repeat":
            print("You already fired there. Pick another cell.")
            continue

        print("You hit a ship!" if result == "hit" else "Splash—miss.")
        if all_sunk(computer_board):
        ...
        if comp_result == "hit":
            print(f"Computer fires at {coord} — it hits!")
        else:
            print(f"Computer fires at {coord} — it misses.")

        if all_sunk(player_board):
            print("\nYour fleet sank. Computer wins.")
            break
